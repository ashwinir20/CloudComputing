	...
	public void map(LongWritable key, Text value, Context context)
		throws IOException, InterruptedException {
			...
			if (rrd.targetUrlsList.size()<=0){
				...
				double rankValOutputUrl = rrd.rankValue/(double)rrd.targetUrlsList.size();
				
				sb.append(String.valueOf(0.0F));		
				
				for(int i=0;i<rrd.targetUrlsList.size();i++){

					targetUrl = rrd.targetUrlsList.get(i).intValue();
					sb.append((new StringBuilder("#").append(targetUrl).toString()));
				    context.write(new LongWritable(targetUrl), new Text(String.valueOf(rankValOutputUrl)));
        		}
